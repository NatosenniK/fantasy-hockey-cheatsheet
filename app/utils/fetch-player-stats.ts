...
export async function FetchPlayerStats(playerId: number) {
    ...
    if (last5Games) {
        last5Games.forEach((game) => {
            ...
            const basePoints = (expGoals + expAssists + expPlusMinus + expPenaltyMinutes + expShots) * recentPerformanceWeight;
    ...
    if (games && prevStats) {
        games.forEach((game) => {
            ...
            if (teamStats) {
                ...
                expectedPointsPastPerformance = basePoints + expPowerPlayPoints + expShorthandedGoals + expShorthandedAssists;
