// Loading animation
const shimmer =
	'before:absolute before:inset-0 before:-translate-x-full before:animate-[shimmer_2s_infinite] before:bg-gradient-to-r before:from-transparent before:via-white/60 before:to-transparent'

export default function PlayerStatSkeleton() {
	return (
		<>
			<div className="mt-6 flow-root w-full">
				<div className="inline-block min-w-full align-middle">
					<div className="p-2 md:pt-0">
						<div className="grid gap-6 md:grid-cols-1 lg:grid-cols-2 mb-6">
							<div className="grid gap-6 grid-cols-2">
								<CardSkeleton />
								<CardSkeleton />
							</div>
							<div className="grid gap-6 grid-cols-1">
								<CardSkeleton />
							</div>
						</div>

						<div className="h-5 w-48 rounded-md bg-gray-200 dark:bg-slate-600 mb-3" />
						<div className={`grid gap-6 md:grid-cols-1 lg:grid-cols-3`}>
							<CardSkeleton />
							<CardSkeleton />
							<CardSkeleton />
						</div>

						<div className="rounded-lg bg-slate-700 mt-4 p-3">
							<div className="h-5 w-48 rounded-md bg-gray-200 dark:bg-slate-600 mb-3" />
							<CardSkeleton />
						</div>
					</div>
				</div>
			</div>
		</>
	)
}

export function CardSkeleton() {
	return (
		<div className={`${shimmer} relative overflow-hidden rounded-xl p-2 shadow-sm bg-slate-700`}>
			<div className="flex p-4">
				<div className="h-5 w-5 rounded-md bg-gray-200 dark:bg-slate-600" />
				<div className="ml-2 h-6 w-16 rounded-md bg-gray-200 text-sm font-medium dark:bg-slate-600" />
			</div>
			<div className="flex items-center justify-center truncate rounded-xl bg-white px-4 py-8 dark:bg-slate-700">
				<div className="h-7 w-20 rounded-md bg-gray-200 dark:bg-slate-600" />
			</div>
		</div>
	)
}

export function FantasyOutlookSkeleton() {
	return (
		<div className={`${shimmer} relative overflow-hidden rounded-xl shadow-sm bg-slate-700`}>
			<div className="flex items-center justify-center truncate rounded-xl bg-white bg-slate-700">
				<div className="h-20 w-full rounded-md bg-gray-200 bg-slate-600 flex-grow" />
			</div>
		</div>
	)
}

export function FantasyComparisonSkeleton() {
	return (
		<div className={`${shimmer} relative overflow-hidden rounded-xl shadow-sm bg-slate-700`}>
			<div className="flex items-center justify-center truncate rounded-xl bg-white bg-slate-700">
				<div className="h-20 w-full rounded-md bg-gray-200 bg-slate-600 flex-grow" />
			</div>
		</div>
	)
}

export function CardsSkeleton() {
	return (
		<>
			<CardSkeleton />
			<CardSkeleton />
			<CardSkeleton />
			<CardSkeleton />
		</>
	)
}

export function TableRowSkeleton() {
	return (
		<tr className={`${shimmer} odd:bg-white even:bg-gray-50 dark:odd:bg-slate-700 dark:even:bg-slate-800`}>
			<td className="px-6 py-4 text-sm text-gray-900 dark:text-white border-b border-gray-200 dark:border-slate-700 flex items-center">
				<div className="h-6 w-5 rounded-md bg-gray-200 text-sm font-medium dark:bg-slate-600" />
			</td>
			<td className="px-6 py-4 text-sm text-gray-900 dark:text-white border-b border-gray-200 dark:border-slate-700">
				<div className="flex items-center">
					<div className="h-5 w-5 rounded-full bg-gray-200 dark:bg-slate-600" />
					<div className="ml-2 h-6 w-24 rounded-md bg-gray-200 text-sm font-medium dark:bg-slate-600" />
				</div>
			</td>

			<td className="px-6 py-4 text-sm text-gray-900 dark:text-white border-b border-gray-200 dark:border-slate-700">
				<div className="h-7 w-5 rounded-md bg-gray-200 dark:bg-slate-600" />
			</td>

			<td className="px-6 py-4 text-sm text-gray-900 dark:text-white border-b border-gray-200 dark:border-slate-700">
				<div className="h-7 w-5 rounded-md bg-gray-200 dark:bg-slate-600" />
			</td>
			<td className="px-6 py-4 text-sm text-gray-900 dark:text-white border-b border-gray-200 dark:border-slate-700">
				<div className="h-7 w-5 rounded-md bg-gray-200 dark:bg-slate-600" />
			</td>
			<td className="px-6 py-4 text-sm text-gray-900 dark:text-white border-b border-gray-200 dark:border-slate-700">
				<div className="h-7 w-5 rounded-md bg-gray-200 dark:bg-slate-600" />
			</td>
			<td className="px-6 py-4 text-sm text-gray-900 dark:text-white border-b border-gray-200 dark:border-slate-700">
				<div className="h-7 w-5 rounded-md bg-gray-200 dark:bg-slate-600" />
			</td>
			<td className="px-6 py-4 text-sm text-gray-900 dark:text-white border-b border-gray-200 dark:border-slate-700">
				<div className="h-7 w-5 rounded-md bg-gray-200 dark:bg-slate-600" />
			</td>
			<td className="px-6 py-4 text-sm text-gray-900 dark:text-white border-b border-gray-200 dark:border-slate-700">
				<div className="h-7 w-5 rounded-md bg-gray-200 dark:bg-slate-600" />
			</td>
			<td className="px-6 py-4 text-sm text-gray-900 dark:text-white border-b border-gray-200 dark:border-slate-700">
				<div className="h-7 w-5 rounded-md bg-gray-200 dark:bg-slate-600" />
			</td>
			<td className="px-6 py-4 text-sm text-gray-900 dark:text-white border-b border-gray-200 dark:border-slate-700">
				<div className="h-7 w-5 rounded-md bg-gray-200 dark:bg-slate-600" />
			</td>
			<td className="px-6 py-4 text-sm text-gray-900 dark:text-white border-b border-gray-200 dark:border-slate-700">
				<div className="h-7 w-5 rounded-md bg-gray-200 dark:bg-slate-600" />
			</td>
			<td className="px-6 py-4 text-sm text-gray-900 dark:text-white border-b border-gray-200 dark:border-slate-700">
				<div className="h-7 w-5 rounded-md bg-gray-200 dark:bg-slate-600" />
			</td>
		</tr>
	)
}
